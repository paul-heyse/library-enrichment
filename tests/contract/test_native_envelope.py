"""The native response envelope accepts and rejects exactly what it is specified to.

schemas/generated/research-envelope.schema.json is the candidate hard-pivot contract.
...
def test_structural_extract_matches_the_native_schema() -> None:
    """tests/fixtures/wire/vocabulary.json is what generated schemas are compared against.

    If it drifts from the contract it was extracted from, every downstream conformance check is
    measuring the wrong thing.
    """
    native = json.loads((ROOT / "tests/fixtures/wire/vocabulary.json").read_text())
    defs = SCHEMA["$defs"]
