...
/// Render a parsed type using the same upstream syntax kernel as complete signatures.
/// Paths use the exact document's path table when no public occurrence is selected.
pub(crate) fn type_rendering(krate: &Crate, value: &Type) -> String {
    ...
    context.render_type(value).iter().map(Token::text).collect()
}

pub(crate) fn generics_rendering(krate: &Crate, value: &Generics) -> String {
    let context = RenderingContext {
        crate_: krate,
        id_to_items: HashMap::new(),
        options: Options {
    ...
    let mut tokens = context.render_generics(value);
    tokens.extend(context.render_where_predicates(&value.where_predicates));
