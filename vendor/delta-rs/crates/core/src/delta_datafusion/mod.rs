...
pub struct DeltaTableFactory {
    opener: Option<Arc<dyn DeltaTableOpener>>,
}

/// Admission seam for an application-owned, exact snapshot and explicit scan contract.
#[async_trait::async_trait]
pub trait DeltaTableOpener: std::fmt::Debug + Send + Sync {
    /// Admit and capture the table and its complete native scan contract.
    async fn open(
        &self,
        session: &dyn Session,
        command: &CreateExternalTable,
    ) -> datafusion::error::Result<(crate::DeltaTable, DeltaScanConfig)>;
}

impl DeltaTableFactory {
    /// Preserve native provider construction after controlled table opening.
    pub fn with_opener(opener: Arc<dyn DeltaTableOpener>) -> Self {
        Self {
            opener: Some(opener),
        }
    }
...
impl TableProviderFactory for DeltaTableFactory {
    async fn create(
        ...
        if let Some(opener) = &self.opener {
            let (table, config) = opener.open(ctx, cmd).await?;
...
mod tests {
    ...
    async fn delta_table_factory_rejects_location_counts_other_than_one() {
        ...
        let factory = DeltaTableFactory::default();
