...
def acquired_dir(manifest: dict, resolved: dict[str, str]) -> Path:
    ...
    version = resolved.get(name, "unknown")
    return ACQUIRED / f"{name}@{version}-{envelope_key(manifest, resolved)}"
...
def assert_tools(manifest: dict) -> dict[str, str]:
    ...
    if expected not in found["ast_grep"]:
        say(f"  ! ast-grep is {found['ast_grep']}, manifest expects {expected}")
    return found
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Acquire normalized API documents for FastMCP.")
