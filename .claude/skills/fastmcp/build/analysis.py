...
def _class_members(documents: dict[str, dict]) -> tuple[dict[str, set[str]], dict[str, set[str]]]:
    ...
    for document in documents.values():
        for item in document["items"]:
            ...
            declared = {
                ...
                if not member.get("inherited") and not member["name"].startswith("_")
            }
...
def assignability(
    binary: Path, config: Path, documents: dict[str, dict], limit: int = 4000
) -> list[list]:
    ...
    """
    protocols, members = _class_members(documents)
    rows: list[list] = []
    asked = 0
...
def capture(
    ...
    satisfies = assignability(binary, config, documents)
