...
# Seeded from a generated inventory -- every trait in the indexed set ranked by how many types
# implement it -- and then narrowed to the ones a caller can actually plug into or must satisfy.
# Ranking first matters: a hand-written list reproduces what the author remembered, and the
# things worth knowing about here (the eight LogStoreFactory implementations, the kernel's
# handler traits) are precisely what nobody remembers.
EXTENSION_POINTS = (
    # Traversing and rewriting the AST: the three flavours differ in ordering and mutability,
    # and choosing the wrong one produces subtly wrong ordering rather than an error.
    "Visitor",
    "SourceOrderVisitor",
    "Transformer",
    "PreorderVisitor",
    # The formatter IR. Implement Format to teach the printer a new node.
    "Format",
    "Buffer",
    "AsFormat",
    "IntoFormat",
    # Positions and source text. Ranged is implemented by 156 types here, which is what makes
    # "give me the span of this thing" uniform across the whole toolchain.
    "Ranged",
    "LineRanges",
    "TextLen",
    "TextSlice",
    ...
    "StringFlags",
    "HasNodeIndex",
    # Caching, configuration and indexing: the seams a downstream embedder implements.
    "CacheKey",
    "OptionsMetadata",
    "Idx",
    # Name resolution.
    "Imported",
    "FutureImport",
    # The checker's own traversal and equality machinery, and the seam for emitting facts.
    "Visit",
    "VisitMut",
    "TypeEq",
    "DisplayWith",
