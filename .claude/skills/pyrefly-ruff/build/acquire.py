...
Usage:
    python3 acquire.py [--manifest manifests/pyrefly-ruff.json] [--check] [--clean]
...
def capsule_root() -> Path:
    ...
    for name in ("PYREFLY_RUFF_ACQUIRE_CAPSULE", "RUST_SKILL_ACQUIRE_CAPSULE"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--stage",
        choices=("rustdoc", "oracles", "all"),
        default="all",
        help="rustdoc needs cargo and a dated nightly; oracles need uv and the pinned binaries",
    )
    ...
    for crate_set in manifest["crate_sets"]:
        if args.stage == "oracles" or crate_set.get("source") != "git":
            continue
        name = crate_set["name"]
        git = crate_set["git"]
        say(f"\n{name} @ {git['rev'][:8]}")
    ...
    if args.stage in ("oracles", "all"):
        oracles.collect(manifest, capsule)
...
if __name__ == "__main__":
    ...
    except (AcquireError, oracles.OracleError) as error:
