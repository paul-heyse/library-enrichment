...
def check_probes(content: Path, probes_path: Path) -> str:
    ...
    for probe in probes["probes"]:
        ...
        # Some of what this repository must get right is an absence: a removed entry point that
        # must not be offered, a feature that was excluded and must therefore contribute no
        # symbols. A "must match" probe cannot express that -- under MULTILINE a negative
        # lookahead passes on the first unrelated line -- so absence gets its own assertion.
        if "reject" in probe and re.search(probe["reject"], haystack, re.MULTILINE):
            hit = re.search(probe["reject"], haystack, re.MULTILINE)
            failures.append(
                f"{probe['question']}: /{probe['reject']}/ matched {hit.group(0)[:60]!r}"
            )
    if failures:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Verify the built ruff/pyrefly repository.")
