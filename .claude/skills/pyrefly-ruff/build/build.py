"""Build the ruff/pyrefly capability repository from pinned upstream sources.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build the ruff/pyrefly capability repository.")
    parser.add_argument("--manifest", type=Path, default=HERE / "manifests" / "pyrefly-ruff.json")
    ...
    # Neither project ships Rust example programs -- their corpora are documentation and a
    # Python conformance suite. Structural linking of Rust trait implementations therefore has
    # nothing to search, and that is a property of these upstreams rather than a misaimed path.
    # It is stated here and recorded in PROVENANCE, because a page that silently reports "no
    # example demonstrates this" is indistinguishable from one whose search was broken.
    ...
    if not usage_roots:
        say("  no Rust example corpus upstream; symbol-to-example edges are not derivable here")
    try:
        demonstrated: dict = {}
        registered: dict = {}
        ...
        if usage_roots and len(demonstrated) < 5:
