...
DataFrame support is experimental and a work in progress. Expect breaking
changes until it stabilizes.
...
ExactReadingFrame = Annotated[pl.DataFrame, Reading]
OpenReadingFrame = Annotated[pl.DataFrame, Reading, ...]

def load_readings() -> ExactReadingFrame:
    return pl.DataFrame(
        {"station": ["North"], "temperature_c": [21.5]},
        schema={"station": pl.String, "temperature_c": pl.Float64},
    )

def temperatures(frame: OpenReadingFrame) -> pl.Series:
    return frame["temperature_c"]

reveal_type(load_readings())  # DataFrame[station: String, temperature_c: Float64]
```

The form without `...` is exact: the argument or return value must have the
same columns, order, and dtypes as the schema class. The form with `...` is
open: the DataFrame may have additional columns in any position, but it must
guarantee every declared column and dtype. An opaque `pl.DataFrame`, or a frame
whose required dtype is unknown, does not satisfy either contract.

`Annotated` is standard Python syntax. Type checkers that do not understand
Pyrefly's DataFrame metadata still see `pl.DataFrame`, and runtime consumers can
inspect the schema class and optional ellipsis with
`get_type_hints(..., include_extras=True)`. Polars itself remains responsible
for runtime schema validation and construction.
