...
accepts a tensor with any number of leading dimensions followed by a `3`,
and returns one with the same leading dimensions followed by a `4` — see
[Variadic dimensions with `Elements`](./tensor-shapes-reference.mdx#variadic-dimensions-with-elements)
in the Reference page for the syntax that expresses this.
...
```python
class Linear[N: IntVar, M: IntVar]:
    def __init__(self, n: Int[N], m: Int[M]):
        ...

    def forward[Xs: IntTuple](self, inp: Tensor[[*Elements[Xs], N]]) -> Tensor[[*Elements[Xs], M]]:
        ...
...
```python
def custom_rand_tensor[A: IntVar, B: IntVar](a: Int[A], b: Int[B]) -> Tensor[[(A + B) // 2]]:
...
```python
def mm[M: IntVar, K: IntVar, N: IntVar](x: Tensor[[M, K]], y: Tensor[[K, N]]) -> Tensor[[M, N]]:
    ...
```
