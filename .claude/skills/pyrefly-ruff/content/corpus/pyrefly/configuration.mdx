...
- Default: Pyrefly adds the `typings/` directory in the project root if it exists.
  Pyrefly then adds entries from
  [Environment Autoconfiguration](#environment-autoconfiguration) if the query succeeds.
- Flag equivalent: `--site-package-path`
- Equivalent configs: none
- Notes:
    - When environment auto-configuration is on, Pyrefly adds entries from the interpreter
      after user-specified `site-package-path` entries. The interpreter entries can include
      paths that `PYTHONPATH` adds to `sys.path`.
...
**`conda-environment`, `python-interpreter-path`, `python-interpreter-find-command`,
...
```toml
python-interpreter-find-command = ["poetry", "env", "info", "-e"]
```
...
A looser, less-strict preset intended for codebases migrating from mypy. This is the preset `pyrefly init` writes when it detects an existing mypy configuration. Pyrefly does not aim to mimic mypy precisely, but the preset preserves selected defaults that would otherwise produce new migration errors.

- Sets [`check-unannotated-defs`](#check-unannotated-defs) `= false`, [`infer-return-types`](#infer-return-types) `= "never"`, [`legacy-overload-expansion`](#legacy-overload-expansion) `= true`, [`replace-untyped-imports-with-any`](#replace-untyped-imports-with-any) `= ["*"]`
- Sets [`type-ignore-unknown-tag-behavior`](#type-ignore-unknown-tag-behavior) `= "suppress"` so existing mypy-coded ignores retain their legacy blanket-suppression behavior.
...
Instruct Pyrefly to replace the given [`ModuleGlob`](#module-globbing)s from
installed third-party packages with `typing.Any` if the module is untyped. Pyrefly considers a
module untyped if it does not find any of:

* A `py.typed` marker in the package.
* A standalone stubs package.

Note that Pyrefly's bundled stubs are ignored for this calculation.

- Type: list of regex
- Default: `[]`
- Flag equivalent: `--replace-untyped-imports-with-any`
- Equivalent config: `follow_untyped_imports = false` in mypy; use `["*"]` to
  apply it to every module. `pyrefly init` converts global and per-module values.
...
- `root` is the absolute path to the root of the repository. `srcs` and
  `buildfile_path` are relative to this root (or to the per-target `root`, if
  set).
...
- [`configs`](#per-target-configuration) is an optional map from a name to a set
  of type checking settings that targets can select with their `config` field.
...
| `python_version` | yes | The target's [`python-version`](#python-version). |
| `python_platform` | yes | The target's [`python-platform`](#python-platform). |
| `deps` | no | List of target names this target depends on. Defaults to `[]`. |
| `buildfile_path` | no | Relative path to the build file that defines this target. Pyrefly treats any of the target's `srcs` outside this file's directory as generated, and does not watch generated files for changes by default. Defaults to `""`. |
| `root` | no | Absolute path override for absolutizing this target's `srcs` and `buildfile_path`. When omitted, the top-level `root` is used. |
| `relative_to` | no | Directory, itself resolved against the target's root, that this target's `srcs` resolve against instead of that root. A build system sets this when it materializes generated sources outside the source tree, such as the output directory of a Buck genrule. Every `src` of a target that sets this counts as generated. |
| `config` | no | Name of an entry in the top-level [`configs`](#per-target-configuration) map, holding the type checking settings for this target. |

#### Per-target configuration

A target's `config` must match a name in the top-level `configs` map. The name
has no meaning to Pyrefly beyond selecting that entry, so the build system can
choose any names that are convenient. Naming configs rather than spelling their
settings inline lets many targets share settings that are sent and parsed only
once.

Each entry in [`configs`](#per-target-configuration) accepts the settings a
[sub-config](#subconfig-allowed-overrides) can override, for example
[`errors`](#errors) and [`check-unannotated-defs`](#check-unannotated-defs).
Target configs take precedence over matching sub-configs because they describe
the most specific build-system scope. The full resolution chain is: target
config → sub-config → top-level config → [preset](#preset) defaults. Settings
that are absent at one level fall through to the next; `errors` maps are merged
so each level only overrides the error kinds it names.
...
`MyLib` and `MyPkg` share the `lenient` config. The test target names no config,
so it is checked with the top-level config.
...
### `type-ignore-unknown-tag-behavior`

Controls how a coded `# type: ignore[...]` comment affects Pyrefly diagnostics
when it contains a tag without the `pyrefly:` prefix. Explicit
`pyrefly:<error-code>` tags remain selective in every mode; unknown tags in the
same comment independently use the configured behavior.

- `no-effect`: The unknown tag has no effect on Pyrefly diagnostics.
- `downgrade-to-warning`: Caps diagnostics on the comment's line at warning severity
  without raising diagnostics that already have a lower severity.
- `suppress`: The comment suppresses all Pyrefly diagnostics on the line, matching
  Pyrefly's legacy behavior.

- Type: `"no-effect" | "downgrade-to-warning" | "suppress"`
- Default: `"suppress"`
- Flag equivalent: `--type-ignore-unknown-tag-behavior`
...
[`python-version`](#python-version) if they're unset. We also use the interpreter's
`sys.path` to get additional [`site-package-path`](#site-package-path) entries. We
search these entries after entries that you configure with CLI flags or a config file.
...
3. Use [`python-interpreter-path`](#python-interpreter-path),
   [`python-interpreter-find-command`](#python-interpreter-find-command),
   [`fallback-python-interpreter-name`](#fallback-python-interpreter-name), or
...
4. Starting from the project root, look for `pyvenv.cfg` in a `.venv`, `venv`, or `env`
   directory. If none is found, repeat the search in each ancestor directory up to the
   filesystem root, preferring the nearest match. Pyrefly infers a project root from the
   files being checked when there is no config file or other root marker. After finding
   `pyvenv.cfg`, Pyrefly uses the interpreter every
   environment builder creates: `bin/python` on macOS and Linux, or
   `Scripts/python.exe` on Windows. Environments in other directories, or that no longer
   have that interpreter and keep only a name such as `python3` or `python3.12`, must be
   activated or set explicitly with
   [`python-interpreter-path`](#python-interpreter-path).
...
- `site-package-path`: entries from `sys.path`. Pyrefly removes the entry that is an empty
  string, entries that contain `.zip`, entries that end in `/lib-dynload`, and any entry
  that is equal to `sysconfig.get_path('stdlib')`.
...
ignore-missing-imports = []
# wildcards for which Pyrefly will replace untyped third-party imports with `typing.Any`
replace-untyped-imports-with-any = []
