...
contains roots from the configuration and the interpreter. Pyrefly uses these roots to
resolve imports.

1. Site package paths from a config file (if no CLI override is present) or CLI args.
2. Filtered entries from a Python interpreter's `sys.path`, if Pyrefly finds an interpreter.
