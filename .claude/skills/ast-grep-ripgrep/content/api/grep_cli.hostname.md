...
Crate `grep-cli` · 1 public items · structured records in [`model/grep_cli.hostname.json`](../model/grep_cli.hostname.json)

## hostname

`function` · `grep_cli::hostname::hostname`

Also reachable as `grep_cli::hostname`

```rust
fn hostname() -> io::Result<std::ffi::OsString>
```
