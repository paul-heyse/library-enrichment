...
Eight checks. Five are the ones every capability repository in this family carries; three exist
because this repository's subjects are programs, and because its most valuable table is curated
rather than derived.
    ...
    router          every recipe in questions.tsv still returns rows, and every probe it cites ran
...
The router check is the one worth explaining. `questions.tsv` is the only table here that is
written rather than extracted, so it is the only one that can quietly stop being true while
everything around it still rebuilds cleanly. A recipe that silently returns nothing is exactly
the failure the whole repository exists to prevent -- a confident pointer to nothing -- so it is
...
def check_determinism() -> dict:
    ...
    It is also the check the behavioural layer makes hardest to pass: probe output carries
    temporary paths, timings and pass numbering. `probes.py` stores matched evidence rather
    than raw output for exactly this reason.
    """
...
def check_rule_tests() -> dict:
    ...
    Two of ast-grep's conventions make a naive gate useless: `ast-grep test` returns 0 even
    when it reports `0 passed; 4 failed`, and `--filter` matching no rule exits 3, so a typo in
    a filter reads as a clean run. Both are asserted on parsed output instead.
    """
    ...
    if passed < len(rule_files) - 1:
        # model-* rules query generated content rather than fixtures.
...
def check_probes() -> dict:
    ...
    for probe in spec["probes"]:
        ...
        if not re.search(probe["expect"], text, re.MULTILINE):
            failures.append(f"{probe['question']}: expected {probe['expect']!r} in {probe['file']}")
        reject = probe.get("reject")
        if reject and re.search(reject, text, re.MULTILINE):
            failures.append(
                f"{probe['question']}: {reject!r} should be absent from {probe['file']}"
            )
    if failures:
...
def check_behaviours() -> dict:
    ...
    The library capability repositories cannot do this: a recorded fact about a crate is a
    ...
    divergent = [r["id"] for r in results if r["verdict"] == "divergent"]
    if divergent:
        raise Failure(
            f"{len(divergent)} probes are divergent: {divergent[:5]}. A divergent probe is one "
            f"whose control did not come out the other way, so it demonstrates nothing."
        )
    return {"behaviours": len(results), "verdicts": probe_module.summarise(results)}


def check_router() -> dict:
    """The curated router still points at rows that exist.

    `questions.tsv` is written, not extracted, so nothing else here would notice it going stale.
    A recipe that returns nothing is a confident pointer to an empty set, which is the precise
    failure this repository is built to prevent.
    """
    rows = (CONTENT / "index" / "questions.tsv").read_text(encoding="utf-8").splitlines()
    probe_ids = {
        line.split("\t")[0]
        for line in (CONTENT / "index" / "behaviors.tsv").read_text(encoding="utf-8").splitlines()
        if line
    ...
    for row in rows:
        if not row:
            continue
        question, _layer, _entry, _rejected, _why, recipe, probe = row.split("\t")
        code, stdout, _ = _run(["sh", "-c", recipe], SKILL_DIR)
        if code == 2 or not stdout.strip():
    ...
    if failures:
        raise Failure(f"{len(failures)} router rows are stale: {failures[:3]}")
...
def _fenced_commands(text: str) -> list[str]:
    ...
    Prose is not a command. `rg is the right tool here` begins with a tool name and is a
    sentence; scraping it and running it produces a usage error that says nothing about the
    documentation being wrong. Only fenced blocks are executable claims.
    """
    commands = []
...
def check_recipes() -> dict:
    ...
    (a clean no-match) is tolerated here; exit 2 from ripgrep or exit 8 from ast-grep is not,
    because both mean the command itself was malformed.
    """
    sources = [SKILL_DIR / "SKILL.md", SKILL_DIR / "reference.md"]
    sources.extend(sorted((CONTENT / "topics").glob("*.md")))
    sources.extend(sorted((CONTENT / "layers").glob("*.md")))
