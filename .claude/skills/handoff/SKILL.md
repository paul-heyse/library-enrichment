---
name: handoff
description: Rewrite STATUS.md from the actual state of the tree, so the next session starts without re-deriving anything. Use at the end of a session that changed what is true, after a phase gate, or when STATUS.md has drifted from the code.
...
3. **Known failures and blocks**, by gate ID or test name, each with the specific prerequisite or
   fix. Known failures stay visible; do not quietly drop one because it is inconvenient.
4. **Next unmet work** — the specific next thing, not a restatement of the phase.
5. **Pinned tool versions** actually in use.

`STATUS.md` is a **state-of-work** record. It is not the design — that is `docs/design/DESIGN.md`,
and a claim about what the service *is* belongs there, amended by an ADR. The 2026-09-13 design
review found `STATUS.md` and `DESIGN.md` disagreeing about which phase the service had reached;
if they disagree again, the tree settles it and both get corrected.
