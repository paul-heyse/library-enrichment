...
GENERATED = ("index", "model", "api", "catalogs", "corpus", "probes", "seams", "topics")
...
def write_indexes(built: model.Model, out: Path) -> dict[str, int]:
    ...
    # `subject` and `reachability` are the two columns that make three subjects one index.
    # Without `subject` a row cannot say WHICH library it describes, and `typer._click.Command`
    # and `click.Command` are different classes with the same name. Without `reachability` the
    # 201 vendored-Click rows say only `nameable: no`, which is true and useless -- you never
    # import a Context, you are handed one.
    symbols = [
        "\t".join(
            (
                item.path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    built = model.stitch(documents, acquisition.get("semantic"), manifest)
    say(
    ...
    counts["seams"] = seams.write_all(
        json.loads(HERE.joinpath("seams.json").read_text()),
        built,
        analysed.get("satisfies", []) if isinstance(analysed, dict) else [],
        content,
        emit.api_file,
    )
    say(f"seams: {counts['seams']} pages")

    # Replayed, never executed: acquisition ran the probes, this stage only projects them.
    probe_payload = render_probes.load(acquired, acquisition)
    counts.update(render_probes.write_all(probe_payload, content))
    counts["behaviors"] = _write_table(
        content.joinpath("index", "behaviors.tsv"), render_probes.behaviors_rows(probe_payload)
    )
    say(f"probes: {counts['probes']} rows, {counts['probe_captures']} captures")

    counts["topics"] = topics.write_all(built, content, HERE.joinpath("topics.json"))
    say(f"topics: {counts['topics']} pages")

    catalog_counts = catalogs.write_all(built, content, HERE.joinpath("catalogs.json"))
    ...
    counts["generated_rules"] = queries.write_all(
        queries.load_rows(HERE.joinpath("catalogs.json")),
        SKILL_ROOT.joinpath("queries", "rules", "generated"),
        SKILL_ROOT.joinpath("queries", "rule-tests"),
    )
