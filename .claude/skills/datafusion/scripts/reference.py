...
def brief(record: dict) -> dict:
    return {
        k: record[k]
        for k in ["id", "title", "brief", "input", "output", "choices", "unknowns", "evidence"]
    } | {
...
def find(root: Path, args: argparse.Namespace) -> dict:
    ...
    registry_file = root / "skill_improvement/evidence/implementation/runtime-registry.json"
    registry = json.loads(registry_file.read_text())
    functions = [
        f
        for f in registry["functions"]
        if args.task and args.task.lower() in f["registered_name"].lower()
    ]
    return {
        "query": args.task,
        ...
        "registered_functions": functions[: args.limit],
