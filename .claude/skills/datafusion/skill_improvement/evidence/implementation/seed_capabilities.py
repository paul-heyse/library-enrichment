...
shutil.copyfile(ROOT / "skill_improvement/examples/crate-roles.tsv", ROOT / "authoring/crate-roles.tsv")


def card(key, title, tasks, crates, inputs, outputs, properties, operations, brief,
         choices, contract, implementation, unknowns, tests=()):
    evidence = [{"id": "upstream", "kind": "upstream_documentation", "operations": operations,
                 "scope": "Pinned hosted rustdoc; availability depends on the consumer profile."}]
    if tests:
        evidence.append({"id": "probe", "kind": "runtime_observation", "tests": list(tests),
    ...
    record = {
        "schema_version": 1, "id": key, "title": title,
        "task_aliases": tasks, "crates": crates,
        "input": inputs, "output": outputs, "properties": properties,
        "operations": operations, "brief": brief,
        "choices": [{"candidate": c, "choose_when": when, "tradeoff": tradeoff,
                     "kind": "authored_conditional_judgment"} for c, when, tradeoff in choices],
        "claims": [{"id": f"{key}.{aspect}", "aspect": aspect, "statement": detail,
                    "kind": "upstream_contract_interpretation", "evidence": ["upstream"],
                    "conditions": ["DataFusion 55.1.0 / Arrow-Parquet 59.3.0 as applicable"]}
                   for aspect, detail in contract],
        "implementation": implementation, "unknowns": unknowns,
        "evidence": evidence, "reviewed": "2026-09-18",
        "invalidation": {"dependencies": operations, "candidate_set": [c[0] for c in choices],
...
card("arrow.select", "Choose a row or column selection representation",
     ["filter rows", "Boolean mask", "gather indices", "reorder duplicate positions", "slice contiguous range", "project columns", "zero columns"],
     ["arrow-select", "arrow-array"], ["RecordBatch", "Array", "BooleanArray", "integer indices"], ["RecordBatch", "ArrayRef"],
     ["nulls", "order", "duplicates", "bounds", "metadata", "zero-column"],
     ["arrow_select::filter::filter_record_batch", "arrow_select::take::take", "arrow_select::take::take_record_batch", "arrow_array::record_batch::RecordBatch::slice", "arrow_array::record_batch::RecordBatch::project", "arrow_array::record_batch::RecordBatch::try_new_with_options"],
     ...
     [("filter_record_batch", "Keep true mask positions in source order", "False and null mask positions are excluded."),
      ("take / take_record_batch", "Gather in requested order, including repeated indices", "Array take exposes checked bounds; the batch helper does not expose options."),
     ...
     ["Choose mask/index/range/column semantics before the API.", "Check indices and output nullability, preserving field/schema metadata deliberately.", "For zero columns, use explicit row-count construction and retain the same index-domain validation."],
     ["Tests cover Int32/UInt32 and tested metadata; nested/view layouts and every datatype are not qualified."],
     ["filter_null_is_not_selected_and_metadata_is_retained", "take_preserves_requested_order_duplicates_and_null_indices", "empty_schema_exposes_different_batch_selection_contracts", "batch_construction_checks_top_level_nullability"])

card("arrow.filter-reuse", "Reuse selection preparation when the workload warrants it",
     ["same mask many arrays", "filter predicate reuse", "filter once", "selection bitmap setup"],
     ["arrow-select"], ["BooleanArray", "Array", "RecordBatch"], ["FilterPredicate", "ArrayRef", "RecordBatch"],
     ["reuse", "nulls", "setup-cost"],
     ["arrow_select::filter::FilterBuilder", "arrow_select::filter::FilterBuilder::optimize", "arrow_select::filter::FilterPredicate::filter", "arrow_select::filter::filter"],
     ...
     ["Reuse one built predicate only for arrays with the intended shared row domain.", "Measure one-shot and repeated cases separately; compare exact outputs before timing."],
...
card("arrow.cast", "Choose conversion failure and output-schema policies explicitly",
     ["parse numeric strings", "cast bad values to null", "reject invalid cast", "strict conversion", "safe cast"],
     ["arrow-cast", "arrow-schema"], ["Array", "DataType", "CastOptions"], ["ArrayRef", "ArrowError"],
     ["nulls", "errors", "coercion", "metadata"],
     ["arrow_cast::cast::cast_with_options", "arrow_cast::cast::CastOptions::safe", "arrow_cast::cast::can_cast_types"],
     ...
     ["Specify failure policy before choosing the output field.", "Check conversion-specific decimal/timezone/nested rules and test malformed, valid and input-null controls."],
...
card("arrow.row-keys", "Encode composite keys for local comparisons",
     ["compare composite keys", "multi column sorting", "dictionary row encoding", "durable key bytes", "row decode"],
     ["arrow-row", "arrow-ord", "arrow-ipc"], ["ArrayRef", "SortField", "SortOptions"], ["Rows", "OwnedRow", "ArrayRef"],
     ["order", "nulls", "dictionary", "lifecycle", "interchange"],
     ["arrow_row::RowConverter", "arrow_row::RowConverter::convert_columns", "arrow_row::RowConverter::convert_rows", "arrow_row::SortField", "arrow_ord::sort::lexsort_to_indices"],
     ...
     ["Keep converter configuration associated with rows.", "Validate decoded physical types and schema separately from value equality.", "Use a specified interchange format when output must outlive the local comparison contract."],
     ["Float/nested/extension domain equivalence and cross-release serialization are not qualified."],
     ["row_order_changes_with_null_placement", "row_decode_hydrates_dictionary_and_preserves_values"])

card("df.consume", "Choose stream, materialization, cache and resource boundaries",
     ["consume incrementally", "large query result", "stream memory budget", "blocking sort", "cache repeat execution", "cancel stream"],
     ["datafusion", "datafusion-execution", "datafusion-physical-plan"], ["DataFrame", "RuntimeEnv"], ["SendableRecordBatchStream", "RecordBatch", "DataFrame"],
     ["streaming", "memory", "spill", "order", "errors", "lifecycle", "reuse"],
     ["datafusion::dataframe::DataFrame::collect", "datafusion::dataframe::DataFrame::execute_stream", "datafusion::dataframe::DataFrame::execute_stream_partitioned", "datafusion::dataframe::DataFrame::cache", "datafusion_execution::runtime_env::RuntimeEnvBuilder::with_memory_limit", "datafusion_execution::disk_manager::DiskManagerBuilder"],
     ...
     ["Choose terminal API from ownership and consumption requirements.", "Inspect blocking operators and configure their resource policy before execution.", "Handle failures both creating and polling streams; test cancellation if operational correctness depends on it."],
...
card("df.source", "Reuse a provider before implementing a new source",
     ["materialized batches table", "partitioned files", "Parquet object storage", "provider view cache", "register table"],
     ["datafusion", "datafusion-catalog", "datafusion-datasource", "datafusion-datasource-parquet", "object_store"],
     ["RecordBatch", "files", "LogicalPlan", "ObjectStore"], ["TableProvider", "DataFrame", "ExecutionPlan"],
     ["reuse", "pruning", "partitioning", "schema", "storage"],
     ["datafusion_catalog::memory::table::MemTable", "datafusion_catalog::view::ViewTable", "datafusion_catalog_listing::table::ListingTable", "datafusion::execution::context::SessionContext::register_object_store", "datafusion_session::table::TableProvider::scan"],
     ...
     ["Select the source representation and lifecycle first.", "Compare built-in provider/format/source combinations.", "Verify read projection, partition schema and predicate behavior on a small source-to-result fixture."],
     ...
     ["provider_exact_inexact_projection_limit_match_reference", "parquet_listing_source_executes_query"])

card("df.pushdown", "Advertise only sound filter, projection and limit pushdown",
     ["custom provider scan", "exact inexact predicate", "min max pruning", "limit projection filter column", "false negatives"],
     ["datafusion-session", "datafusion-expr", "datafusion-physical-plan"], ["Expr", "projection indices", "limit", "Session"], ["ExecutionPlan", "ScanResult"],
     ["nulls", "duplicates", "exactness", "projection", "limit", "pruning"],
     ["datafusion_session::table::TableProvider::scan", "datafusion_session::table::TableProvider::supports_filters_pushdown", "datafusion_session::table::TableProvider::scan_with_args", "datafusion_expr::table_source::TableProviderFilterPushDown"],
     ...
     ["Evaluate support per expression, not just by predicate column name.", "Keep predicate inputs available through filtering, then apply safe limit and output projection.", "Differentially compare Unsupported/reference execution with Exact/Inexact; inspect residual plans and scan arguments."],
...
card("df.expressions", "Separate expression construction, coercion and evaluation",
     ["build expression", "incompatible types", "simplify direct expression", "coalesce constructor", "type coercion"],
     ["datafusion-expr", "datafusion-optimizer", "datafusion-physical-expr", "datafusion-functions"], ["Expr", "DFSchema", "ScalarUDF"], ["Expr", "PhysicalExpr", "ColumnarValue"],
     ["coercion", "nulls", "errors", "planning"],
     ["datafusion_expr::udf::ScalarUDF::call", "datafusion_optimizer::simplify_expressions::expr_simplifier::ExprSimplifier::coerce", "datafusion_optimizer::simplify_expressions::expr_simplifier::ExprSimplifier::simplify", "datafusion::execution::context::SessionContext::create_physical_expr"],
     ...
     ["Identify whether the caller enters normal analysis or bypasses it.", "For direct processing, coerce with the intended schema, then simplify and plan.", "Check both output datatype and value; include an unsupported pair control."],
...
card("parquet.selection", "Compose row-group pruning, row selection and decode filters",
     ["Parquet row selection", "skip row groups", "page index", "predicate decode", "retained row coordinates"],
     ["parquet", "datafusion-datasource-parquet"], ["Parquet bytes", "row groups", "RowSelection", "RowFilter"], ["RecordBatch", "reader"],
     ["pruning", "coordinates", "projection", "io", "metadata"],
     ["parquet::arrow::arrow_reader::ArrowReaderBuilder::with_row_groups", "parquet::arrow::arrow_reader::ArrowReaderBuilder::with_row_selection", "parquet::arrow::arrow_reader::ArrowReaderBuilder::with_row_filter", "parquet::arrow::arrow_reader::selection::RowSelection"],
     ...
     ["Choose groups first, then generate/rebase selection coordinates.", "Retain predicate columns until evaluation even when absent from output.", "Compare row identities against an unpruned reader; instrument I/O separately for performance claims."],
...
card("df.functions", "Discover built-ins in the actual session before writing a UDF",
     ["built in scalar functions", "nested list array map", "SQL aliases", "table function", "return field volatility", "custom UDF alternative"],
     ["datafusion-functions", "datafusion-functions-nested", "datafusion-functions-aggregate", "datafusion-functions-window", "datafusion-functions-table", "datafusion-expr"],
     ["Expr", "ColumnarValue", "SessionState"], ["Expr", "ColumnarValue", "TableProvider"],
     ["registry", "features", "coercion", "volatility", "nulls", "metadata"],
     ["datafusion::execution::session_state::SessionState::scalar_functions", "datafusion::execution::session_state::SessionState::aggregate_functions", "datafusion::execution::session_state::SessionState::window_functions", "datafusion::execution::session_state::SessionState::table_functions", "datafusion_expr::udf::ScalarUDFImpl"],
     ...
     ["Find a SQL name or task synonym, resolve its defining implementation and expr_fn helper.", "Check actual registry membership and Cargo features.", "Read the specific function's signature, coercion and return-field/null contract before composing."],
...
card("df.relations", "Choose relational cardinality, null-key and ordering semantics",
     ["join duplicates", "null keys join", "union all distinct", "set bag operations", "sort limit top k", "unnest list"],
     ["datafusion", "datafusion-expr", "datafusion-physical-plan", "datafusion-functions-nested"], ["DataFrame", "Expr"], ["DataFrame", "LogicalPlan"],
     ["cardinality", "duplicates", "nulls", "order", "partitioning"],
     ["datafusion::dataframe::DataFrame::join", "datafusion::dataframe::DataFrame::join_on", "datafusion::dataframe::DataFrame::union", "datafusion::dataframe::DataFrame::union_distinct", "datafusion::dataframe::DataFrame::intersect", "datafusion::dataframe::DataFrame::except", "datafusion::dataframe::DataFrame::unnest_columns"],
     ...
     ["Write down expected multiplicity, null equality and tie behavior.", "Use duplicate/null/empty inputs in a differential fixture.", "Inspect explain output for sort/top-k and cardinality effects; validate values independently of plan shape."],
...
card("arrow.schema", "Distinguish batch validity, relational schema and metadata propagation",
     ["align schema", "extension metadata", "non nullable field", "zero column row count", "physical expression adapter", "schema evolution"],
     ["arrow-schema", "arrow-array", "datafusion-common", "datafusion-physical-expr-adapter"], ["Schema", "Field", "ArrayRef", "DFSchema"], ["RecordBatch", "DFSchema", "PhysicalExpr"],
     ["schema", "metadata", "nulls", "validation", "projection"],
     ["arrow_array::record_batch::RecordBatch::try_new", "arrow_array::record_batch::RecordBatch::try_new_with_options", "arrow_schema::field::Field", "datafusion_common::dfschema::DFSchema", "datafusion_physical_expr_adapter::schema_rewriter::DefaultPhysicalExprAdapter"],
     ...
     ["Specify which metadata is contractual versus advisory.", "Carry or reconstruct fields explicitly when kernels return only arrays.", "Probe each consequential logical-to-physical operator boundary, including schema plus values."],
     ...
     ["batch_construction_checks_top_level_nullability", "filter_null_is_not_selected_and_metadata_is_retained"])

card("df.aggregate-window", "Reuse aggregate/window functions and characterize state before extensions",
     ["aggregate merge state", "groups accumulator", "sliding window retract", "window frame peers", "partial aggregation", "count distinct"],
     ["datafusion-functions-aggregate", "datafusion-functions-aggregate-common", "datafusion-functions-window", "datafusion-expr", "datafusion-expr-common"],
     ["ArrayRef", "group indices", "window partition", "state arrays"], ["ScalarValue", "state arrays", "ArrayRef"],
     ["state", "merge", "retract", "order", "nulls", "frames"],
     ["datafusion_expr::udaf::AggregateUDFImpl", "datafusion_expr_common::accumulator::Accumulator", "datafusion_expr_common::groups_accumulator::GroupsAccumulator", "datafusion_expr::partition_evaluator::PartitionEvaluator", "datafusion_functions_aggregate_common::aggregate::groups_accumulator::GroupsAccumulatorAdapter"],
     ...
     ["Look up the built-in function and its signature/null/frame behavior.", "For custom aggregates test partitioned partial->merge against single-pass output.", "For moving windows test duplicate keys, empty frames and retractions; inspect state schema and memory reporting."],
...
card("df.storage-reuse", "Separate source reuse, plan reuse, materialization and writes",
     ["repeat query", "reuse view cache", "write Parquet files", "object store registry", "metadata cache", "insert overwrite"],
     ["datafusion", "datafusion-execution", "datafusion-datasource", "object_store", "parquet"], ["DataFrame", "ObjectStore", "TableProvider"], ["files", "DataFrame", "ExecutionPlan"],
     ["reuse", "storage", "lifecycle", "configuration", "errors"],
     ["datafusion::dataframe::DataFrame::cache", "datafusion::dataframe::DataFrame::into_view", "datafusion::dataframe::DataFrame::write_parquet", "datafusion::dataframe::DataFrame::write_table", "datafusion_execution::runtime_env::RuntimeEnv", "datafusion_session::table::TableProvider::insert_into"],
     ...
     ["Specify exactly which computation/data/metadata is reused and when it becomes stale.", "Verify writer and source ownership, registry lifetime and output schema.", "Round-trip a small written file; separately qualify atomicity or cloud failure guarantees if required."],
     ...
     ["stream_collect_cache_and_sort_resource_error", "parquet_listing_source_executes_query"])

card("df.interchange", "Match the boundary to data, plan or ABI interchange",
     ["serialize plan", "Substrait", "protobuf custom codec", "Arrow IPC Flight", "FFI ownership"],
     ["datafusion-proto", "datafusion-substrait", "datafusion-ffi", "arrow-ipc", "arrow-flight"],
     ["LogicalPlan", "ExecutionPlan", "RecordBatch", "FFI provider"], ["bytes", "plan", "stream", "FFI handles"],
     ["interchange", "registry", "ownership", "compatibility"],
     ["datafusion_proto::logical_plan::LogicalExtensionCodec", "datafusion_proto::physical_plan::PhysicalExtensionCodec"],
     ...
     ["Identify whether the payload is data, a plan, or in-process handles.", "List extension codecs and registry/store dependencies.", "Test restoration and execution in the receiver environment."],
