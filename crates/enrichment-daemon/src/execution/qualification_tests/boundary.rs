...
use crate::execution::{Runner, cleanup::Supervisor};
...
"#;
    ...
    runner
        ...
        .await
        .unwrap();
    assert!(runner.ownership.reservations().await.unwrap().is_empty());
    assert!(runner.ownership.active_owners().await.unwrap().is_empty());
}
    ...
    for (compiler, success) in [("1.98.1", false), (rustdoc::TOOLCHAIN, true)] {
        ...
        if success {
            assert!(
                identity
                    .stdout
                    .contains(enrichment_core::execution::producer::RUSTDOC_RELEASE)
            );
            assert!(
                identity
                    .stdout
                    .contains(enrichment_core::execution::producer::RUSTDOC_COMMIT)
            );
        } else {
