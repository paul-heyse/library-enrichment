...
async fn owned_names(store: &enrichment_store::physical_ownership::OwnershipStore) -> Vec<String> {
    let mut names: Vec<_> = store
        .active_owners()
        .await
        .unwrap()
        .into_iter()
        .map(|owner| owner.owner_id.broker_name())
        .collect();
    names.sort();
    names
}

/// Active native owners captured by this fixture; absent observations remain durable history.
async fn owned_since(
    store: &enrichment_store::physical_ownership::OwnershipStore,
    before: &[String],
) -> Vec<String> {
    owned_names(store)
        .await
...
async fn until(deadline: Duration, mut condition: impl AsyncFnMut() -> bool) -> bool {
...
async fn a_dropped_execution_task_still_removes_its_container_and_descendants() {
    ...
    .unwrap();
    let ownership = runner.ownership.clone();
    let before = owned_names(&ownership).await;
    ...
    let live = until(Duration::from_secs(15), async || {
        owned_since(&ownership, &before)
            .await
            .first()
    ...
    if !live {
        task.abort();
        let _ = until(Duration::from_secs(30), async || supervisor.is_idle()).await;
        panic!("target heartbeat never became observable in live scratch");
    }
    let names = owned_since(&ownership, &before).await;
    assert_eq!(names.len(), 1, "exactly one owned container: {names:?}");
    let name = names[0].clone();
    let first = heartbeat(&root, &name).unwrap();
    assert!(
        until(Duration::from_secs(3), async || heartbeat(&root, &name)
    ...
    assert!(
        until(Duration::from_secs(120), async || supervisor.is_idle()).await,
    ...
    assert!(
        owned_since(&ownership, &before).await.is_empty(),
        "the native owner may become absent only after confirmed physical absence"
    );
...
async fn a_failed_removal_is_reported_truthfully_and_quarantines_admission() {
    ...
    let leaked = owned_since(&ownership, &before).await;
    assert_eq!(
        leaked.len(),
        1,
    ...
    assert!(
        until(Duration::from_secs(180), async || supervisor.is_idle()).await,
    ...
    assert!(
        owned_since(&ownership, &before).await.is_empty(),
        "no active native owner may remain once absence is confirmed"
    );
...
async fn cancellation_before_start_runs_no_target_code_and_restart_reconciles_an_orphan() {
    ...
    assert!(owned_since(&ownership, &before).await.is_empty());
    ...
    let image = config.python_image.as_ref().unwrap();
    let (qualified, operation) = runner
        .fixture_request(
            image,
            capsule.path(),
        ...
        .await
        .unwrap();
    let dispatch = qualified
        .authorize_process(image, &operation, false)
        .await
        .unwrap();
    qualified
        .reserve_owner(&name, capsule.path(), image, &operation, &dispatch)
        .await
        .unwrap();
    ownership
        .observe(
            &name,
            super::OwnershipObservation::Creating {
                boot_id: super::boot_id().unwrap(),
            },
        )
        .await
        .unwrap();
    ...
    ownership
        .observe(
            &name,
            super::OwnershipObservation::Settled {
                boot_id: super::boot_id().unwrap(),
            },
        )
        .await
        .unwrap();
    assert!(container_present(&root, &name.broker_name()));
    ...
    assert!(
        !container_present(&root, &name.broker_name()),
        "startup reconciliation must confirm the orphan is gone"
    );
    assert!(!owned_names(&ownership).await.contains(&name.broker_name()));
}
...
async fn a_dropped_or_timed_out_creator_cannot_outlive_its_cleanup_ownership() {
    ...
    for abort in [true, false] {
        ...
        let started = until(Duration::from_secs(10), async || entered.exists()).await;
        if abort {
            task.abort();
        }
        ...
        let settled = until(Duration::from_secs(30), async || supervisor.is_idle()).await;
...
async fn creation_past_the_operation_deadline_never_starts_the_target() {
    ...
    assert!(until(Duration::from_secs(15), async || supervisor.is_idle()).await);
    assert!(
        !started.exists(),
