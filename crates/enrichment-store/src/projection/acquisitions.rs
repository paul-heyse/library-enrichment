...
pub(super) fn array(rows: &[&[Artifact]]) -> Result<ArrayRef, ArrowError> {
    let flat: Vec<_> = rows.iter().flat_map(|r| r.iter()).collect();
    record_list(rows.iter().map(|r| r.len()), values(&flat)?)
}

pub(super) fn values(flat: &[&Artifact]) -> Result<ArrayRef, ArrowError> {
    for a in flat {
        validate(a)?;
    }
    structure(
...
pub(super) fn decode(row: Row<'_>) -> Result<Vec<Artifact>, ArrowError> {
...
pub(super) fn decode_one(r: Row<'_>) -> Result<Artifact, ArrowError> {
    let a = Artifact {
