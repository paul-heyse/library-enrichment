...
impl AttemptPlan {
    ...
    pub(crate) fn input(runtime: &QueryRuntime, attempts: Attempts) -> Result<Self> {
        ...
        let runs = provenance::producer_fields(
        ...
        let frame = crate::native_catalog::batch(&runtime.session(), "attempt_ingress", batch)?
    ...
    pub(crate) async fn validate(&self, runtime: &QueryRuntime) -> Result<()> {
        crate::producer_run_plan::admit(runtime, &self.0).await?;
    ...
    pub(crate) async fn logs(&self, runtime: &QueryRuntime) -> Result<Vec<Artifact>> {
        ...
        let session = runtime.session();
        crate::native_catalog::work(&session, "attempt_inputs", self.0.clone().into_view())?;
        let logs = session.sql("WITH receipts AS (SELECT log,unnest(acquisitions) AS artifact FROM attempt_inputs WHERE log IS NOT NULL) SELECT DISTINCT artifact FROM receipts WHERE log=artifact.artifact_id").await?;
...
pub(crate) async fn reference_rules(
    ...
    let logs = session.sql(&format!("WITH receipts AS (SELECT {key} AS id,unnest(acquisitions) AS artifact FROM {table}) SELECT a.{key} FROM {table} a LEFT ANTI JOIN receipts r ON a.{key}=r.id AND a.log=r.artifact.artifact_id WHERE a.log IS NOT NULL LIMIT 1")).await?;
    invariants.push(logs, "attempt_log_reference", "publication")?;
    let inputs = session.sql(&format!("WITH receipts AS (SELECT {key} AS id,unnest(acquisitions) AS artifact FROM {table}), inputs AS (SELECT {key} AS id,unnest(native_map_entries(inputs)) AS input FROM {table}) SELECT i.id FROM inputs i LEFT ANTI JOIN receipts r ON i.id=r.id AND i.input.value=r.artifact.sha256 LIMIT 1")).await?;
    invariants.push(inputs, "attempt_input_reference", "publication")
}
