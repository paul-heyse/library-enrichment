...
pub(crate) async fn build(
    ...
    bases: crate::native_catalog::BoundCatalog,
    ...
    for (base, bound, surface, full) in [
        ...
        for (name, sql) in [
            (bound, binding_sql(base)),
            (surface, surface_sql(bound, full)),
        ] {
            let session = runtime.bound_session(std::collections::BTreeMap::from([(
                "snapshot".into(),
                std::sync::Arc::new(bases.clone().with_schema(
    ...
    for (name, sql) in [
        ...
        (
            ...
            FROM snapshot.evidence.symbols
        ",
        ),
        (
            "lexical_ancestry",
        ...
        (
            ...
            FROM snapshot.domain.lexical_ancestry WHERE depth < path_depth
        ",
        ),
        (
            "path_nodes",
            ...
            FROM snapshot.domain.lexical_ancestry
        ",
        ),
        (
            "namespace_children",
            ...
            FROM snapshot.domain.namespace_members m JOIN snapshot.domain.api_surface s ON m.symbol_id = s.symbol_id
            WHERE m.is_direct
        ",
        ),
        (
            ...
            FROM snapshot.domain.namespace_members
            UNION
            SELECT ecosystem, components, CAST(array_length(components) AS BIGINT) AS depth, path
            FROM snapshot.domain.api_surface WHERE kind = 'module'
        ",
        ),
        (
            "fragment_surface",
        ...
        (
            ...
            FROM snapshot.evidence.api_observations a JOIN snapshot.evidence.input_artifacts i
            ...
            FROM snapshot.evidence.fragments f JOIN snapshot.evidence.input_artifacts i
            ...
            FROM snapshot.evidence.relationships r JOIN snapshot.evidence.input_artifacts i
            ...
            FROM snapshot.evidence.release_metadata m JOIN snapshot.evidence.input_artifacts i
        ...
        (
            ...
            FROM snapshot.domain.api_surface s
        ",
        ),
    ] {
        let session = runtime.bound_session(std::collections::BTreeMap::from([(
...
fn surface_sql(bound: &str, full: bool) -> String {
    ...
    format!("SELECT s.*, d.kind, d.definition_path, d.defined_in_package, a.observation_id, a.origin, a.environment_id, a.payload, a.source {aliases}
        FROM snapshot.evidence.symbols s JOIN snapshot.evidence.definitions d ON s.definition_id = d.definition_id LEFT JOIN snapshot.domain.{bound} a ON s.symbol_id = a.binding_id")
}
